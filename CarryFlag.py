...
#Make the program stop as soon as it overwrites a value from the original program instead of when it hits the bottom line of the original file (which could be empty)

#Carry flag

#The carry flag (C) is a single bit used to indicate when an arithmetic carry or borrow has been generated out of the most significant bit position. This occurs in prog1 as the program generates a value larger than MAX_INT = 127.
...
def ConvertToBinary(DecimalNumber):
  ...
  return BinaryString
#-------------------------------------------------------------------------------------------------------------------------#
def StatusConvertToBinary(DecimalNumber):
  BinaryString = EMPTY_STRING
  while DecimalNumber > 0:
    Remainder = DecimalNumber % 2
    Bit = str(Remainder)
...
def DisplayCurrentState(SourceCode, Memory, Registers):
  ...
  print("*")
  print("*  PC: ", Registers[PC], " ACC: ", Registers[ACC], " TOS: ", Registers[TOS], "CAR: ", Registers[CAR])
  print(f"Registers: {Registers}")
  print("*  Status Register: SZNV")
  #-------------------------------------------------------------------------------------------------------------------------#
  print("*                  ", StatusConvertToBinary(Registers[STATUS]))
  #-------------------------------------------------------------------------------------------------------------------------#
...
def StatusFlag(Value, Registers):
  #---------------------------------------------------------------------------------------------------------------------------#
...
def StackOverflowError(ErrorMessage, Registers):
